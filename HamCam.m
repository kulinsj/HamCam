function [ easyPulse, HamCamPulse] = HamCam(infileName,LEDX, LEDY, validationMinPeakDist, expected, rangeRadius, hOffset)
...
%         imshow(videoFrame);
        ...
        ExtrapolatedPoint(i,1:3,1:2) = [eyePairBigX + eyePairBigW/5, eyePairBigY + eyePairBigH*hOffset; ...
            eyePairBigX + 4*eyePairBigW/5, eyePairBigY + eyePairBigH*hOffset; ...
...
%         figure('name',strcat(' Red LED signal for  ', infileName));
        actualPeaks = size(LEDpeaks,2);
        easyPulse = 60*actualPeaks*30/numFrames;
%         plot(redLED);
%         hold on;
...
%             figure('name',strcat('Processed heartbeat from sample ', num2str(i), ' for face', num2str(k)));
%             plot(1:size(G,2),G(:),'color',[1 0 0]);
%             hold on;
%             ylim([0, 1]);
%             legend(strcat(num2str(flow*60),' to ', num2str(fhigh*60)));
            [peaks, ~] = findpeaks(G,'MINPEAKDISTANCE',10);
%             scatter(locs, G(locs));
...
%         plot(1:(size(F,2)-1),abs(F(:,2:end)));
        ...
        HamCamPulse = f(freq(pulseInd)+1)*60;
